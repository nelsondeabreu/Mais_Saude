/*
  ...
  - You are about to drop the column `postID` on the `partilha` table. All the data in the column will be lost.
  - Added the required column `createPostShareID` to the `Partilha` table without a default value. This is not possible if the table is not empty.
  - Added the required column `postShareID` to the `Partilha` table without a default value. This is not possible if the table is not empty.
...
-- DropIndex
DROP INDEX `Partilha_postID_fkey` ON `partilha`;

-- DropIndex
DROP INDEX `Partilha_userID_fkey` ON `partilha`;
...
-- AlterTable
ALTER TABLE `partilha` DROP COLUMN `postID`,
    ADD COLUMN `createPostShareID` VARCHAR(191) NOT NULL,
    ADD COLUMN `postShareID` VARCHAR(191) NOT NULL;
