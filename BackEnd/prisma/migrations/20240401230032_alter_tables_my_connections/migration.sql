/*
  ...
  - You are about to drop the column `email` on the `minhasconexoes` table. All the data in the column will be lost.
  - You are about to drop the column `nome` on the `minhasconexoes` table. All the data in the column will be lost.
  - Added the required column `friendID` to the `MinhasConexoes` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE `minhasconexoes` DROP COLUMN `email`,
    DROP COLUMN `nome`,
