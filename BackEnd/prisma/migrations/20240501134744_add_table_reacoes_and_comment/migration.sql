/*
  ...
  - Added the required column `updatedAt` to the `Publicacoes` table without a default value. This is not possible if the table is not empty.
...
-- DropIndex
DROP INDEX `TempoNotificacao_consultaID_fkey` ON `temponotificacao`;

-- AlterTable
ALTER TABLE `publicacoes` ADD COLUMN `updatedAt` DATETIME(3) NOT NULL;
...
-- AddForeignKey
ALTER TABLE `TempoNotificacao` ADD CONSTRAINT `TempoNotificacao_consultaID_fkey` FOREIGN KEY (`consultaID`) REFERENCES `Consultas`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
