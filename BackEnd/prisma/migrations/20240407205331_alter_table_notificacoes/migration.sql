/*
  ...
  - You are about to drop the column `conteudo` on the `notificacoes` table. All the data in the column will be lost.
  - Added the required column `consultaID` to the `Notificacoes` table without a default value. This is not possible if the table is not empty.
  - Added the required column `tipo` to the `Notificacoes` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE `notificacoes` DROP COLUMN `conteudo`,
    ADD COLUMN `consultaID` VARCHAR(191) NOT NULL,
