...
export default function CardPost({image,legend,name,id,userPhoto,crm,data,autorID,video,postID,share}){
    ...
    React.useEffect(()=>{
        ...
        socket && socket.on("allComments",(data)=>{
    ...
    async function sharePost(e){
        const req1 = (await api.post("posts/",{conteudo:shareRef.current.value,usuarioID:user.id,})).data
        ...
        <Col12>
            <nav className={style.container}>
                <div className={style.divGeral}>
                    ...
                    {
                        image && <img className={style.imagePost} src={`http://localhost:2004/posts/${image}/findPostImage`} alt="" />
                    }
                    
                    {
                        video && <VideoPlayer videoSource={`http://localhost:2004/posts/${video}/findPostVideo`}/>
                    }
                        {
                            !share && (
                        ...
                        <Popover.Root>
                            
                            {
                                allComments.length <= 2 ? (
                                    ...
                                    <Dialog.Root>
                                        {/** Caso o comentario seja mais de 3*/}
                                                                    ...
                                                                    <Dialog.Portal>
                                                                        <Dialog.Content className={style.shareContainer}>
                                                                            ...
                                                                            <div className={style.body}>
                                                                                ...
                                                                                <button onClick={sharePost}>Enviar</button>
                                                                                </form>
                                                                            </div>
                                                                        </Dialog.Content>
                                                                    </Dialog.Portal>
